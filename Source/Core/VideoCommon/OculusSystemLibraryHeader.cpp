...
static PFUNC_HMD_HSW ovrHmd_GetHSWDisplayState;
...
PFUNC_SESSIONSTATUS ovr_GetSessionStatus;
PFUNC_UINT_HMD ovr_GetTrackingCaps;
...
static bool Load8()
{
  TLibOvrVersion v = libovr_080;
#ifdef _WIN64
  dll = LoadLibraryA("LibOVRRT64_0_8.dll");
#else
  dll = LoadLibraryA("LibOVRRT32_0_8.dll");
#endif
  if (!dll)
    return false;
  bool b = true;
...
static bool Load6()
{
  ...
  b = b && (ovrHmd_CreateMirrorTextureD3D11 =
                (PFUNC_CREATEMIRRORD3D116)GetProcAddress(dll, "ovrHmd_CreateMirrorTextureD3D11"));
  b = b && (ovrHmd_CreateMirrorTextureGL =
                (PFUNC_CREATEMIRRORGL)GetProcAddress(dll, "ovrHmd_CreateMirrorTextureGL"));
  b = b && (ovrHmd_CreateSwapTextureSetD3D11 =
                (PFUNC_CREATESWAPD3D116)GetProcAddress(dll, "ovrHmd_CreateSwapTextureSetD3D11"));
  b = b && (ovrHmd_CreateSwapTextureSetGL =
                (PFUNC_CREATESWAPGL)GetProcAddress(dll, "ovrHmd_CreateSwapTextureSetGL"));
  b = b && (ovrHmd_Destroy = (PFUNC_VOID_HMD)GetProcAddress(dll, "ovrHmd_Destroy"));
  b = b && (ovrHmd_DestroyMirrorTexture =
                (PFUNC_DESTROYMIRROR)GetProcAddress(dll, "ovrHmd_DestroyMirrorTexture"));
  b = b && (ovrHmd_DestroySwapTextureSet =
                (PFUNC_DESTROYSWAP)GetProcAddress(dll, "ovrHmd_DestroySwapTextureSet"));
  ...
  b = b &&
      (ovrHmd_GetFloatArray = (PFUNC_GETFLOATARRAY)GetProcAddress(dll, "ovrHmd_GetFloatArray"));
  ...
  b = b &&
      (ovrHmd_SetFloatArray = (PFUNC_SETFLOATARRAY)GetProcAddress(dll, "ovrHmd_SetFloatArray"));
...
static bool Load5()
{
  ...
  b = b && (ovrHmd_GetTrackingState =
                (PFUNC_TRACKING_STATE5)GetProcAddress(dll, "ovrHmd_GetTrackingState"));
  b = b && (ovrHmd_GetFovTextureSize = (PFUNC_FOV)GetProcAddress(dll, "ovrHmd_GetFovTextureSize"));
  b = b &&
  ...
  b = b && (ovrHmd_DismissHSWDisplay_Real =
                (PFUNC_CHAR_HMD)GetProcAddress(dll, "ovrHmd_DismissHSWDisplay"));
  b = b &&
...
ovrHmdDescComplete ovr_GetHmdDesc(ovrHmd hmd)
{
  ...
  else if (ovr_GetHmdDesc_Real && g_libovr_version >= libovr_070)
  {
    ...
    result.DistortionCaps = ovrDistortionCap_Chromatic | ovrDistortionCap_TimeWarp |
  ...
  else if (g_libovr_version == libovr_060)
  {
    ...
    result.AvailableTrackingCaps = desc.TrackingCaps | ovrTrackingCap_Orientation |
